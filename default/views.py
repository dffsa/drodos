...
def index(request):
    ...
    if request.user.is_authenticated:
        ...
        return render(request, 'default/home.html', {'files': files, 'user': request.user})
    else:
...
def loginp(request):
    if request.user.is_authenticated:
        return render(request, 'default/home.html', {'user': request.user})
    else:
...
def logoutp(request):
    logout(request)
    return render(request, 'default/index.html')
    HttpResponseRedirect(reverse('default:index'))
...
def checklogin(request):
    ...
    if user is not None:
        ...
        return HttpResponseRedirect(reverse('default:index'))
    else:
        # invalid login
        return HttpResponseRedirect(reverse('default:loginp'))
...
def checkregister(request):
    if not (User.objects.filter(username=request.POST['username']).exists() or User.objects.filter(
        ...
        if form.is_valid():
            ...
            user.save()
            return HttpResponseRedirect(reverse('default:registered'))
        else:
            return HttpResponseRedirect(reverse('default:registerfail', ))
    else:
        return HttpResponseRedirect(reverse('default:registerfail', ))
...
def registerfail(request):
    if request.user.is_authenticated:
        ...
        form = RegisterForm()
        return render(request, 'default/registerfail.html', {'form': form})
...
def registered(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'default/registered.html')
...
def upload(request):
    if request.method == 'POST':
        ...
        if not request.FILES:
            if request.user.is_authenticated:
                return render(request, 'default/home.html', {'erro': 'no file selected', 'user': request.user})
            else:
                return render(request, 'default/index.html', {'erro': 'no file selected'})
        else:
            ...
            if request.user.is_authenticated:
                if request.user.profile.currentStorage + size > request.user.profile.maxStorage:  # max storage reached
                    fs.delete(filename)
                    return HttpResponseRedirect(reverse('default:myprofile'))
                    ...
                    request.user.save()
                    storeditem.save()
                    return HttpResponseRedirect(reverse('default:files'))
                ...
                if size > 1000:  # too big for unregistered
                    ...
                    tempitem.save()
                    return HttpResponseRedirect(reverse('default:uploaded', args=(filename,)))
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def uploaded(request, filename):
    if request.user.is_authenticated:
        ...
        return render(request, 'default/uploaded.html', {'filename': filename})
...
def files(request):
    if request.user.is_authenticated:
        files = StoredItem.objects.filter(owner=request.user)
        return render(request, 'default/files.html', {'files': files})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def file(request, filename):
    ...
    if request.user.is_authenticated:
        if storeditem.owner == request.user:
            return render(request, 'default/file.html', {'storeditem': storeditem})
        else:
            if not storeditem.private or storeditem.sharedwith(request.user):
                return render(request, 'default/viewfile2.html', {'storeditem': storeditem})
            else:
                return HttpResponseRedirect(reverse('default:index'))
    else:
        if storeditem.private:
            ...
            return render(request, 'default/viewfile.html', {'storeditem': storeditem})


def groups(request):
    if request.user.is_authenticated:
        groups = Group.objects.filter(member=request.user)
        return render(request, 'default/groups.html', {'groups': groups, 'user': request.user})
    else:
        return render(request, 'default/index.html')
    ...
    if request.user.is_authenticated:
        ...
        if group.ismember(request.user):
            ...
            if group.isowner(request.user):  # group owner
                return render(request, 'default/groupowner.html', {'group': group, 'useritems': useritems})
            else:  # groupmember
                return render(request, 'default/group.html', {'group': group, 'useritems': useritems})
        else:  # not a member
...
def viewgroup(request, groupname):
    if request.user.is_authenticated:
        group = get_object_or_404(Group, name=groupname)
        return render(request, 'default/viewgroup.html', {'group': group})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def managegroups(request):
    if request.user.is_authenticated:
        ...
        if request.POST['op'] == 'create':
            return HttpResponseRedirect(reverse('default:creategroup'))
        elif request.POST['op'] == 'inv':
            return HttpResponseRedirect(reverse('default:invites'))
        else:  # invalid op
            return HttpResponseRedirect(reverse('default:groups', args=(groups,)))
    else:
        return render(request, 'default/index.html')
...
def invitemember(request, groupname):
    ...
    if request.user.is_authenticated and group.isowner(request.user):
        return render(request, 'default/groupowner.html', {'inviteform': True, 'group': group})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def removemember(request, groupname):
    group = get_object_or_404(Group, name=groupname)
    if request.user.is_authenticated and group.isowner(request.user):
        return render(request, 'default/groupowner.html', {'removeform': True, 'group': group})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def addfile(request, groupname):
    ...
    if request.user.is_authenticated and group.ismember(request.user):
        ...
        if group.isowner(request.user):
            return render(request, 'default/groupowner.html', {'addfileform': True, 'group': group,
                                                              'useritems': useritems})
        else:
            return render(request, 'default/group.html', {'addfileform': True, 'group': group,
                                                         'useritems': useritems})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def removefile(request, groupname):
    ...
    if request.user.is_authenticated and group.ismember(request.user):
        useritems = StoredItem.objects.filter(owner=request.user)
        if group.isowner(request.user):
            return render(request, 'default/groupowner.html', {'removefileform': True, 'group': group,
                                                              'useritems': useritems})
        else:
            usergroupitems = []
            ...
            return render(request, 'default/group.html', {'removefileform': True, 'group': group,
...
def changesettings(request, filename):
    ...
    if request.user == storeditem.owner and request.user.profile.premium:
        if storeditem.private:
            ...
            storeditem.save()
            return render(request, 'default/file.html', {'changed': 'public', 'storeditem': storeditem})
        else:
            storeditem.private = True
            storeditem.save()
            return render(request, 'default/file.html', {'changed': 'private', 'storeditem': storeditem})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def leavegroup(request, groupname):
    ...
    if request.user.is_authenticated and group.ismember(request.user):
        ...
        print('left group')
        return HttpResponseRedirect(reverse('default:groups'))
    else:  # impossible
        return HttpResponseRedirect(reverse('default:index'))
...
def removedmember(request, groupname):
    ...
    if request.user.is_authenticated and group.isowner(request.user):
        if not request.POST.get('user'):
            return HttpResponseRedirect(reverse('default:group', args=(groupname,)))
        else:
    ...
    if request.user.is_authenticated and group.isowner(request.user):
        if User.objects.filter(username=request.POST['username']).exists():
            ...
            if group.ismember(invited):
                ...
                return render(request, 'default/groupowner.html',
...
def creategroup(request):
    if request.user.is_authenticated and request.user.profile.premium:
        form = GroupForm()
        return render(request, 'default/creategroup.html', {'form': form})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def invites(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'default/invites.html', {'invites': invites})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def answerinvite(request, groupid):
    if request.user.is_authenticated:
        ...
        if request.POST['op'] == 'Accept':
            ...
            return HttpResponseRedirect(reverse('default:managegroups'))
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def savegroup(request):
    if not Group.objects.filter(name=request.POST['name']).exists():
        ...
        if form.is_valid():
            ...
            print(form.errors)
            return render(request, 'default/creategroup.html', {'form': form, 'erro': 'Group name already taken!'})
    else:
        form = GroupForm()
        return render(request, 'default/creategroup.html', {'form': form, 'erro': 'Group name already taken!'})
...
def user(request, username):
    if request.user.is_authenticated:
        if request.user.username == username:
            ...
            return render(request, 'default/user.html', {'user': user})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def myprofile(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'default/myprofile.html', {'user': user})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def premium(request):
    if request.user.is_authenticated:
        return render(request, 'default/premium.html', {'user': user})
    else:
        return HttpResponseRedirect(reverse('default:index'))
...
def testing(request):
    return render(request, 'default/testing.html')